...
from transformers import GPT2Tokenizer, GPT2LMHeadModel, DistilBertTokenizer, DistilBertModel
...
def compute_semantic_relatedness(question: str, ground_truth: str, output: str) -> float:
    try:
...
def compute_token_based_metrics(ground_truth: str, generated_output: str) -> Tuple[float, float]:
    ...
    f1 = f1_score(ground_truth_tokens, generated_output_tokens, average='weighted', zero_division=0)
    recall = recall_score(ground_truth_tokens, generated_output_tokens, average='weighted', zero_division=0)
...
class MetricsCalculator:
    ...
    def compute_perplexity(self, text: str) -> float:
        try:
            ...
            with torch.no_grad():
                outputs = self.model_manager.gpt2_model(**inputs, labels=inputs['input_ids'])
                loss = outputs.loss
    ...
    def compute_coherence(self, text: str) -> float | floating[Any]:
        try:
            ...
            embeddings = [self.model_manager.distilbert_model(**self.model_manager.distilbert_tokenizer(token, return_tensors="pt")).last_hidden_state.mean(dim=1).squeeze().detach().numpy() for token in tokens]
            if len(embeddings) < 2:
                return 0.0
            pairwise_similarities = [cosine_similarity([embeddings[i]], [embeddings[i + 1]])[0][0] for i in range(len(embeddings) - 1)]
    ...
    @staticmethod
    def compute_factual_accuracy(output: str, question: str, ground_truth: str) -> float:
        try:
            keywords = set(question.split() + ground_truth.split())
            output_words = set(output.split())
            return len(keywords.intersection(output_words)) / len(keywords) if keywords else 0.0
        except Exception as e:
    ...
    def compute_all_metrics(self, question: str, ground_truth: str, output: str) -> Dict[str, float]:
        return {
            'cosine_similarity': self.compute_similarity(question + ground_truth, output),
            ...
            'semantic_relatedness': compute_semantic_relatedness(question, ground_truth, output),
            'factual_accuracy': self.compute_factual_accuracy(output, question, ground_truth),
...
class Evaluator:
    ...
    def compute_metrics(self, question: str, ground_truth: str, output: str) -> Dict[str, float]:
        try:
            return self.metrics_calculator.compute_all_metrics(question, ground_truth, output)
        except Exception as e:
            logger.error(f"Error in compute_metrics: {e}")
            return {}

    def evaluate_output(self, output: str, question: str, ground_truth: str) -> Tuple[str, Dict[str, float]]:
        return output, self.compute_metrics(question, ground_truth, output)

    def hybrid_contextual_evaluation(self, question: str, ground_truth: str, generated_output: str) -> Tuple[float, float, float]:
        try:
            generated_outputs = [generated_output]

            with ThreadPoolExecutor() as executor:
                future_to_output = {executor.submit(self.evaluate_output, output, question, ground_truth): output for output in generated_outputs}
                scores = [future.result() for future in as_completed(future_to_output)]

            best_output, best_combined_score = max(scores, key=lambda x: sum(x[1].values()))

            final_similarity = self.metrics_calculator.compute_similarity(best_output, generated_output) * 100
...
def main() -> None:
    ...
    final_similarity, f1_score, recall_score = evaluator.hybrid_contextual_evaluation(question, ground_truth, generated_output)
    ...
    table_data = [[metric.replace('_', ' ').title(), f"{score:.2f}"] for metric, score in combined_metrics.items()]
